...
export class CoreServicesClient {
   ...
   */
  private getCoreServicesEndpoint(): string {
   ...
   */
  private async getInstanceDetailsREST(instanceId: string): Promise<InstanceDetails> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }

    try {
      const response = await this.restClient['httpClient'].get(
        `${this.getCoreServicesEndpoint()}/20160918/instances/${instanceId}`
      );
   ...
   */
  private async getInstanceDetailsCLI(instanceId: string): Promise<InstanceDetails> {
    try {
   ...
   */
  async performInstanceAction(instanceId: string, action: string): Promise<InstanceAction> {
   ...
   */
  private async performInstanceActionREST(instanceId: string, action: string, previousState: string): Promise<InstanceAction> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }
    ...
    try {
      ...
      const response = await this.restClient['httpClient'].post(
        `${this.getCoreServicesEndpoint()}/20160918/instances/${instanceId}/actions/${action.toLowerCase()}`,
        payload
      );
   ...
   */
  private async performInstanceActionCLI(instanceId: string, action: string, previousState: string): Promise<InstanceAction> {
    try {
   ...
   */
  async getVolumeAttachments(instanceId: string, compartmentId: string): Promise<VolumeAttachment[]> {
    if (this.useRestAPI && this.restClient) {
      try {
   ...
   */
  private async getVolumeAttachmentsREST(instanceId: string, compartmentId: string): Promise<VolumeAttachment[]> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }
    ...
    try {
      const response = await this.restClient['httpClient'].get(
        `${this.getCoreServicesEndpoint()}/20160918/volumeAttachments?compartmentId=${compartmentId}&instanceId=${instanceId}&limit=1000`
      );
   ...
   */
  private async getVolumeAttachmentsCLI(instanceId: string, compartmentId: string): Promise<VolumeAttachment[]> {
    try {
   ...
   */
  async getVnicAttachments(instanceId: string, compartmentId: string): Promise<VnicAttachment[]> {
    if (this.useRestAPI && this.restClient) {
      try {
   ...
   */
  private async getVnicAttachmentsREST(instanceId: string, compartmentId: string): Promise<VnicAttachment[]> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }
    ...
    try {
      const response = await this.restClient['httpClient'].get(
        `${this.getCoreServicesEndpoint()}/20160918/vnicAttachments?compartmentId=${compartmentId}&instanceId=${instanceId}&limit=1000`
      );

      const attachments: VnicAttachment[] = [];
   ...
   */
  private async getVnicAttachmentsCLI(instanceId: string, compartmentId: string): Promise<VnicAttachment[]> {
    try {
   ...
   * Get VNIC details
   */
  async getVnicDetails(vnicId: string): Promise<VnicDetails> {
    if (this.useRestAPI && this.restClient) {
      try {
   ...
   */
  private async getVnicDetailsREST(vnicId: string): Promise<VnicDetails> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }
    ...
    try {
      const response = await this.restClient['httpClient'].get(
        `${this.getCoreServicesEndpoint()}/20160918/vnics/${vnicId}`
      );
   ...
   */
  private async getVnicDetailsCLI(vnicId: string): Promise<VnicDetails> {
    try {
   ...
   */
  async waitForInstanceState(instanceId: string, desiredState: string, maxWaitTimeMinutes: number = 10): Promise<{ success: boolean; currentState: string; message: string }> {
   ...
   */
  async listInstances(compartmentId?: string, lifecycleState: string = 'RUNNING'): Promise<InstanceDetails[]> {
    if (this.useRestAPI && this.restClient) {
      try {
   ...
   */
  private async listInstancesREST(compartmentId?: string, lifecycleState: string = 'RUNNING'): Promise<InstanceDetails[]> {
    if (!this.restClient) {
      throw new Error('REST client not initialized');
    }
    ...
    try {
      const response = await this.restClient['httpClient'].get(
        `${this.getCoreServicesEndpoint()}/20160918/instances?compartmentId=${encodeURIComponent(targetCompartmentId)}&lifecycleState=${lifecycleState}&limit=1000`
      );

      const instances: InstanceDetails[] = [];

      if (response.data && Array.isArray(response.data)) {
        response.data.forEach((instanceData: any) => {
          instances.push({
            id: instanceData.id,
   ...
   */
  private async listInstancesCLI(compartmentId?: string, lifecycleState: string = 'RUNNING'): Promise<InstanceDetails[]> {
    const targetCompartmentId = compartmentId || process.env.OCI_COMPARTMENT_ID;
    if (!targetCompartmentId) {
      throw new Error('Compartment ID is required');
    }
   ...
   */
  async listInstancesWithNetworkInfo(compartmentId?: string, lifecycleState: string = 'RUNNING'): Promise<Array<InstanceDetails & { 
    networkInfo: Array<{
      isPrimary: boolean;
      privateIp: string;
      publicIp?: string;
   ...
   */
  async getInstanceFullDetails(instanceId: string, compartmentId?: string): Promise<{
