...
export class MQLQueryEngine {
  ...
  parseMQL(queryString: string): MQLQuery {
    ...
    if (joinMatch) {
      query.joinOperator = joinMatch[2] as '&&' | '||';
    ...
    if (comparisonMatch) {
      query.alarmConditions = [{
        operator: comparisonMatch[1] as any,
   ...
   */
  private parseFilters(dimensionString: string): MQLFilter[] {
    const filters: MQLFilter[] = [];
    ...
    for (const part of parts) {
      ...
      if (opMatch) {
        ...
        let operator = opMatch[2].trim() as any;
        let value: string | string[] | number = opMatch[3].trim();
