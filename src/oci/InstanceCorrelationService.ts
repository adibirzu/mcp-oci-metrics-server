...
export class InstanceCorrelationService {
  ...
  private instanceCache: Map<string, InstanceInfo> = new Map();
  private correlationCache: Map<string, CorrelationMapping> = new Map();
   ...
   */
  async getComputeInstances(compartmentId?: string): Promise<InstanceInfo[]> {
   ...
   */
  private async getInstancesViaREST(compartmentId: string): Promise<InstanceInfo[]> {
    if (!this.restClient) {
    ...
    try {
      ...
      const instancesResponse = await this.restClient['httpClient'].get(
        `https://iaas.${this.restClient.getRegion()}.oraclecloud.com/20160918/instances?compartmentId=${compartmentId}&limit=1000`
      );
      ...
      if (instancesResponse.data && Array.isArray(instancesResponse.data)) {
        for (const instanceData of instancesResponse.data) {
          ...
          const vnicResponse = await this.restClient['httpClient'].get(
            `https://iaas.${this.restClient.getRegion()}.oraclecloud.com/20160918/vnicAttachments?compartmentId=${compartmentId}&instanceId=${instanceData.id}&limit=100`
          );
          ...
          if (vnicResponse.data && Array.isArray(vnicResponse.data)) {
            for (const vnicAttachment of vnicResponse.data) {
              if (vnicAttachment.vnicId) {
                // Get VNIC details
                const vnicDetailResponse = await this.restClient['httpClient'].get(
                  `https://iaas.${this.restClient.getRegion()}.oraclecloud.com/20160918/vnics/${vnicAttachment.vnicId}`
                );
   ...
   */
  private async getInstancesViaCLI(compartmentId: string): Promise<InstanceInfo[]> {
    try {
      // Get compute instances
   ...
   */
  async generateCorrelationMappings(compartmentId?: string): Promise<CorrelationMapping[]> {
    const instances = await this.getComputeInstances(compartmentId);
    const correlations: CorrelationMapping[] = [];
    ...
    for (const instance of instances) {
      ...
      const correlation: CorrelationMapping = {
        ocid: instance.ocid,
   ...
   */
  async findInstancesByIP(ipAddresses: string[], compartmentId?: string): Promise<CorrelationMapping[]> {
    const correlations = await this.generateCorrelationMappings(compartmentId);
    
    return correlations.filter(correlation => 
      ipAddresses.some(ip => 
        correlation.loganCorrelationData.ipAddresses.includes(ip)
      )
    );
  }
   ...
   */
  async findInstancesByName(namePattern: string, compartmentId?: string): Promise<CorrelationMapping[]> {
    const correlations = await this.generateCorrelationMappings(compartmentId);
    const pattern = namePattern.toLowerCase();
    
    return correlations.filter(correlation => 
      correlation.instanceName.toLowerCase().includes(pattern) ||
      correlation.loganCorrelationData.searchPatterns.some(searchPattern => 
        searchPattern.toLowerCase().includes(pattern)
      )
    );
  }
   ...
   */
  async getInstanceByOCID(ocid: string, compartmentId?: string): Promise<CorrelationMapping | null> {
    const correlations = await this.generateCorrelationMappings(compartmentId);
   ...
   */
  async generateLoganCorrelationData(
   ...
   */
  getCacheStats(): { instances: number; correlations: number; lastUpdate: number; isValid: boolean } {
    return {
