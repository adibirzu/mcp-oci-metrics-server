...
async function handleQueryComputeAgentMetrics(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    const result = await monitoringClient.queryComputeAgentMetrics(
      args.instanceId,
      args.metricName,
      timeRange,
      args.interval || 'PT1M'
...
async function handleQueryStackMonitoringMetrics(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    const result = await monitoringClient.queryStackMonitoringMetrics(
      args.resourceId,
      args.metricName,
      timeRange
    );
...
async function handleQueryDbManagementMetrics(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    const result = await monitoringClient.queryDbManagementMetrics(
      args.databaseId,
      args.metricName,
      timeRange
    );
...
async function handleQueryOpsInsightsMetrics(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    const result = await monitoringClient.queryOpsInsightsMetrics(
      args.resourceId,
      args.metricName,
      timeRange
    );
...
async function handleGetInstanceCorrelations(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleFindInstancesByIP(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleFindInstancesByName(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleGenerateLoganCorrelationData(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    ...
    const correlationData = await correlationService.generateLoganCorrelationData(
      timeRange,
      args.compartmentId
    );
      ...
      try {
        ...
        for (const correlation of correlations.slice(0, 5)) { // Limit to first 5 to avoid timeout
          try {
            ...
            const cpuResult = await monitoringClient.queryComputeAgentMetrics(
              correlation.ocid,
              'CpuUtilization',
              timeRange,
              'PT5M'
...
async function handleGetInstanceDetails(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleStartInstance(args: any) {
  if (process.env.MCP_DEBUG) {
  ...
  try {
    ...
    if (args.waitForRunning) {
      stateResult = await coreServicesClient.waitForInstanceState(
        args.instanceId, 
        'RUNNING', 
        args.maxWaitMinutes || 10
      );
    }
...
async function handleStopInstance(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleRestartInstance(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleGetInstanceConsoleHistory(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleGetInstanceVnics(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleGetInstanceVolumes(args: any) {
  if (process.env.MCP_DEBUG) {
...
async function handleWaitForInstanceState(args: any) {
  if (process.env.MCP_DEBUG) {
    console.error('⏳ Waiting for instance state:', JSON.stringify(args, null, 2));
  }

  try {
    const result = await coreServicesClient.waitForInstanceState(
      args.instanceId,
      args.desiredState,
      args.maxWaitMinutes || 10
    );
...
async function handleListComputeInstances(args: any) {
  if (process.env.MCP_DEBUG) {
    console.error('📋 Listing compute instances:', JSON.stringify(args, null, 2));
  }

  try {
    const instances = await coreServicesClient.listInstances(
      args.compartmentId,
      args.lifecycleState || 'RUNNING'
    );
...
async function handleListInstancesWithNetwork(args: any) {
  if (process.env.MCP_DEBUG) {
    console.error('🌐 Listing instances with network info:', JSON.stringify(args, null, 2));
  }

  try {
    const instances = await coreServicesClient.listInstancesWithNetworkInfo(
      args.compartmentId,
      args.lifecycleState || 'RUNNING'
    );
