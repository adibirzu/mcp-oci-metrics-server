...
export class TemplateVariableEngine {
    ...
    constructor(authManager, monitoringClient) {
     ...
     */
    async resolveVariableQuery(variable) {
     ...
     */
    async executeVariableQuery(query) {
     ...
     */
    async getCompartmentOptions(region) {
        try {
            ...
            if (result.data && Array.isArray(result.data)) {
                return result.data
                    .filter((comp) => comp['lifecycle-state'] === 'ACTIVE')
                    .map((comp) => ({
                    text: comp.name,
                    value: comp.id
                }));
     ...
     */
    async getNamespaceOptions(compartmentId) {
        try {
     ...
     */
    async getInstanceOptions(compartmentId) {
        try {
            const compartment = compartmentId || process.env.OCI_COMPARTMENT_ID;
            if (!compartment) {
                throw new Error('No compartment specified');
            }
            const instances = await this.authManager.executeOCICommand(`compute instance list --compartment-id "${compartment}"`);
            const result = JSON.parse(instances.stdout);
            if (result.data && Array.isArray(result.data)) {
                return result.data
                    .filter((instance) => instance['lifecycle-state'] === 'RUNNING')
                    .map((instance) => ({
     ...
     * Get metric options
     */
    async getMetricOptions(namespace) {
        try {
            if (!namespace) {
     ...
     */
    async getDimensionOptions(namespace, metricName) {
        try {
     ...
     */
    extractParameterFromQuery(query, paramIndex) {
     ...
     */
    applyRegexFilter(options, regex) {
        if (!regex) {
            return options;
        }
        try {
            const regexPattern = new RegExp(regex, 'i');
            return options.filter(option => regexPattern.test(option.text) || regexPattern.test(option.value));
        }
        catch (error) {
     ...
     */
    isValidCurrentValue(current, options) {
...
//# sourceMappingURL=TemplateVariableEngine.js.map