...
export class OCIRestClient {
     ...
     */
    parseConfigSection(content, profile) {
    ...
    async queryMonitoringMetrics(query) {
        try {
            ...
            const response = await this.httpClient.post(`${endpoint}/20180401/metrics/actions/summarizeMetricsData`, payload);
    ...
    async listNamespaces(compartmentId) {
        try {
            ...
            const response = await this.httpClient.get(`${endpoint}/20180401/metrics?compartmentId=${targetCompartmentId}&limit=1000`);
    ...
    async listMetrics(namespace, compartmentId) {
        try {
            ...
            const response = await this.httpClient.get(`${endpoint}/20180401/metrics?compartmentId=${targetCompartmentId}&namespace=${namespace}&limit=1000`);
            if (response.data && Array.isArray(response.data)) {
     ...
     */
    async queryComputeAgentMetrics(instanceId, metricName, timeRange, interval = 'PT1M') {
        const query = {
     ...
     */
    async queryStackMonitoringMetrics(resourceId, metricName, timeRange) {
        try {
            ...
            const response = await this.httpClient.post(`${endpoint}/20210330/metrics/actions/summarizeMetricsData`, payload);
     ...
     */
    async queryDbManagementMetrics(databaseId, metricName, timeRange) {
        try {
            ...
            const response = await this.httpClient.get(`${endpoint}/20201101/managedDatabases/${databaseId}/metrics`, {
                params: {
                    startTime: moment(timeRange.startTime).toISOString(),
                    endTime: moment(timeRange.endTime).toISOString(),
                    metricName
     ...
     */
    async queryOpsInsightsMetrics(resourceId, metricName, timeRange) {
        try {
            ...
            const response = await this.httpClient.post(`${endpoint}/20200630/databaseInsights/actions/summarizeDatabaseInsightResourceStatistics`, payload);
            return response.data;
        }
        catch (error) {
...
//# sourceMappingURL=OCIRestClient.js.map