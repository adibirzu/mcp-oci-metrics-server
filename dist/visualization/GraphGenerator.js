...
export class GraphGenerator {
     ...
     */
    async generateLLMCompatibleVisualization(metricData, config) {
        try {
            const asciiChart = this.generateASCIIChart(metricData, config);
            const dataTable = this.generateDataTable(metricData);
            const summary = this.generateSummary(metricData, config, metricData.reduce((sum, metric) => sum + metric.aggregatedDatapoints.length, 0));
     ...
     */
    generateASCIIChart(metricData, config) {
     ...
     */
    async generateGraph(metricData, config) {
        try {
            ...
            const totalDataPoints = metricData.reduce((sum, metric) => sum + metric.aggregatedDatapoints.length, 0);
     ...
     */
    convertToPlotlyData(metricData, config) {
        const traces = [];
        metricData.forEach((metric, index) => {
     ...
     */
    createPlotlyLayout(config, metricData) {
        const layout = {
            title: {
                text: config.title,
        ...
        if (config.type !== 'pie') {
            ...
            const allValues = metricData.flatMap(m => m.aggregatedDatapoints.map(p => p.value));
     ...
     */
    generatePlotlyHTML(data, layout, config) {
...
<body>
     ...
     */
    async generateCorrelationHeatmap(metricData, title = 'Metric Correlation Heatmap') {
        try {
     ...
     */
    calculateCorrelationMatrix(metricData) {
        const matrix = [];
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let j = 0; j < n; j++) {
                if (i === j) {
                    ...
                    matrix[i][j] = this.calculateCorrelation(metricData[i].aggregatedDatapoints, metricData[j].aggregatedDatapoints);
                }
            }
        }
        return matrix;
     ...
     */
    alignTimestamps(series1, series2) {
        const aligned = [];
     ...
     */
    extractTimeRange(metricData) {
        const allTimestamps = metricData.flatMap(metric => metric.aggregatedDatapoints.map(point => point.timestamp));
     ...
     */
    generateSummary(metricData, config, totalDataPoints) {
